...
public class UIWrapContent : MonoBehaviour
{
    ...
    public bool sortDeactiveObject = true;
    ...
    public UIGrid.Sorting Sorting = UIGrid.Sorting.Vertical;
    ...
    protected LockUpdateItem lockUpdateItem = LockUpdateItem.NONE;
    ...
    {
        if (Sorting == UIGrid.Sorting.Alphabetic) SortAlphabetically();
        else
        {
    ...
    public void SortBasedOnScrollMovement()
    {
        ...
        {
            if (sortDeactiveObject)
            {
                mChildren.Add(mTrans.GetChild(i));
            }
            else
            {
                if (NGUITools.GetActive(mTrans.GetChild(i).gameObject))
                {
                    mChildren.Add(mTrans.GetChild(i));
                }
            }
    ...
    public void ResetChildPositions()
    {
        ...
        for (int i = 0, imax = mChildren.Count; i < imax; ++i)
        {
            ...
            t.localPosition = mHorizontal ? new Vector3(i*itemSize, 0f, 0f) : new Vector3(0f, -i*itemSize*dir, 0f);
            UpdateItem(t, i);
        }
    }
        ...
        else
        {
            ...
            for (int i = 0, imax = mChildren.Count; i < imax; ++i)
            {
                ...
                if (distance < -extents)
                {
                    ...
                    if (minIndex == maxIndex || (minIndex <= realIndex && realIndex <= maxIndex))
                    {
                        UpdateItemPivot pivot = UpdateItemPivot.TOP;
                        if (CheckUpdateItem(i, pivot))
                        {
                            t.localPosition = pos;
                            UpdateItem(t, i, pivot);
                        }
                        else
                        {
                ...
                else if (distance > extents)
                {
                    ...
                    if (minIndex == maxIndex || (minIndex <= realIndex && realIndex <= maxIndex))
                    {
                        UpdateItemPivot pivot = UpdateItemPivot.BOTTOM;
                        if (CheckUpdateItem(i, pivot))
                        {
                            t.localPosition = pos;
                            UpdateItem(t, i, pivot);
                ...
                else if (mFirstTime) UpdateItem(t, i, UpdateItemPivot.BOTTOM);
    ...
    //protected virtual void UdateItem(Transform item, int index)
    //{
    ...
    /// </summary>
    protected virtual void UpdateItem(Transform item, int index, UpdateItemPivot pivot)
    {
    ...
    protected virtual bool CheckUpdateItem(int index, UpdateItemPivot pivot)
    {
        return true;
    }

    public enum UpdateItemPivot
    {
        LEFT,
        RIGHT,
        TOP,
