...
public class UITable : UIWidgetContainer
{
    ...
    public Vector2 paddingEachItem = Vector2.zero;
    ...
    protected void RepositionVariableSize(List<Transform> children)
    {
        ...
        for (int i = 0, imax = children.Count; i < imax; ++i)
        {
            ...
            pos.x = xOffset + b.extents.x - b.center.x + paddingEachItem.x*i;
            pos.x -= Mathf.Lerp(0f, b.max.x - b.min.x - br.max.x + br.min.x, po.x) - padding.x;

            if (direction == Direction.Down)
            {
                pos.y = -yOffset - b.extents.y - b.center.y - paddingEachItem.y*i;
            ...
            if (sortDepth)
            {
                ...
                {
                    if (depth == 0)
                        depth = t.GetComponent<UIWidget>().depth;
                }
                else
                {
                    depth--;
