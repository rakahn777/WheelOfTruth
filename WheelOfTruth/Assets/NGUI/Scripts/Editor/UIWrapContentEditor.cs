...
public class UIWrapContentEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        GUILayout.BeginHorizontal();
        SerializedProperty sp1 = NGUIEditorTools.DrawProperty("Range Limit", serializedObject, "minIndex",
        ...
        NGUIEditorTools.DrawProperty("Sort Deactive Object", serializedObject, "sortDeactiveObject");
        NGUIEditorTools.DrawProperty("Sorting ", serializedObject, "Sorting");
        NGUIEditorTools.DrawProperty("Is Reverse", serializedObject, "isReverse");
        NGUIEditorTools.DrawProperty("Anchor Object", serializedObject, "anchor");
        ...
        if (sp1.intValue != sp2.intValue)
        {
            if ((target as UIWrapContent).GetComponent<UICenterOnChild>() != null)
            {
                EditorGUILayout.HelpBox(
                    "Limiting indices doesn't play well with UICenterOnChild. You should either not limit the indices, or not use UICenterOnChild.",
                    MessageType.Warning);
            }
        }
    }
